...
export const requestConnectionV2 = onCall(async (request) => {
  ...
  const connections: Connections = (await getDatabase().ref('/connection').once('value')).val() || {};
  ...
  await getDatabase().ref('/connection').child(randomWaitingUid).child('watching').set(myUid);
...
export const countConnectionsV2 = onValueWritten('/connection', async (event) => {
  ...
  await getDatabase().ref('/stats').child('connections').child('count').set(count);
});
